...
Namespace Scheduler13._1_TestDesignTime
	Partial Public Class AppointmentFormOutlook2007Style
		...
		Private saveImage As Image
		Private saveAndCloseImage As Image
		Private deleteImage As Image
		Private spellingImage As Image
		Private recurrenceImage As Image
		Private closeLargeImage As Image
		Private saveLargeImage As Image
		Private saveAndCloseLargeImage As Image
		Private deleteLargeImage As Image
		Private spellingLargeImage As Image
		Private recurrenceLargeImage As Image
        ...
        Private Sub LoadFormData(ByVal appointment As DevExpress.XtraScheduler.Appointment)
            Me.SearchLookUpEdit1.Properties.DataSource = BD.RetornaDataTable("Select *, cast(ID_Parte as nvarchar(20)) + ' - ' + Nombre as Descripcio  From C_CalendarioTecnico_Partes_Pendientes  Order by ID_Parte desc")
            Me.SearchLookUpEdit1.Properties.DisplayMember = "Descripcio"
            Me.SearchLookUpEdit1.Properties.ValueMember = "ID_Parte"

            'Dim _pepe As CustomFieldCollection
            '_pepe = appointment.CustomFields
            'If IsDBNull(_pepe("IDParte")) = True OrElse IsNothing(_pepe("IDParte")) Then

            'Else
            '    ' Me.SearchLookUpEdit1.EditValue = _pepe("IDParte")
            'End If

            Me.SearchLookUpEdit1.EditValue = appointment.PercentComplete
            'appointment.CustomFields(0)
            'Me.Storage.Appointments(0).cus
        End Sub
        ...
        Private Function SaveFormData(ByVal appointment As DevExpress.XtraScheduler.Appointment) As Boolean
            ' appointment.CustomFields("IDParte") = Me.SearchLookUpEdit1.EditValue
            'appointment.PercentComplete = Me.SearchLookUpEdit1.EditValue
            Return True
        End Function
		...
		Protected Friend Overridable Sub LoadImages()
			...
			Me.saveImage = LoadImageFromResource(AppointmentFormOutlook2007StyleImagesName.Save)
			Me.saveLargeImage = LoadImageFromResource(AppointmentFormOutlook2007StyleImagesName.SaveLarge)
			Me.saveAndCloseImage = LoadImageFromResource(AppointmentFormOutlook2007StyleImagesName.SaveAndClose)
			Me.saveAndCloseLargeImage = LoadImageFromResource(AppointmentFormOutlook2007StyleImagesName.SaveAndCloseLarge)
			Me.deleteImage = LoadImageFromResource(AppointmentFormOutlook2007StyleImagesName.Delete)
			Me.deleteLargeImage = LoadImageFromResource(AppointmentFormOutlook2007StyleImagesName.DeleteLarge)
			Me.recurrenceImage = LoadImageFromResource(AppointmentFormOutlook2007StyleImagesName.Recurrence)
			Me.recurrenceLargeImage = LoadImageFromResource(AppointmentFormOutlook2007StyleImagesName.RecurrenceLarge)
			Me.closeImage = LoadImageFromResource(AppointmentFormOutlook2007StyleImagesName.Close)
			Me.closeLargeImage = LoadImageFromResource(AppointmentFormOutlook2007StyleImagesName.CloseLarge)
			Me.spellingImage = LoadImageFromResource(AppointmentFormOutlook2007StyleImagesName.Spelling)
			Me.spellingLargeImage = LoadImageFromResource(AppointmentFormOutlook2007StyleImagesName.SpellingLarge)
		End Sub
        ...
        Protected Friend Overridable Sub OnSaveButton()
            'Dim _pepe As CustomFieldCollection
            '_pepe = sourceAppointment.CustomFields
            'Dim _pepe2 As CustomFieldCollection
            '_pepe2 = control_Renamed.SelectedAppointments(0).CustomFields
            'If Me.SearchLookUpEdit1.Tag Is Nothing Then
            '    _pepe("IDParte") = 0
            '    _pepe2("IDParte") = 0
            'Else
            '    _pepe("IDParte") = Me.SearchLookUpEdit1.Tag
            '    _pepe2("IDParte") = Me.SearchLookUpEdit1.Tag
            'End If
            ...
            If (Not Me.SaveFormData(Me.sourceAppointment)) Then
                Return
            End If
            'mira aqui baix ña skfj lñskajfd lñaskj fñlkasj fdlñkaesjfd
            controller_Renamed.PercentComplete = Me.SearchLookUpEdit1.EditValue
...
'ORIGINAL LINE: bool showControls = /*Controller.IsNewAppointment ||*/ editedAptCopy.Type != AppointmentType.Pattern;
        ...
        Private Sub SearchLookUpEdit1_EditValueChanged(sender As Object, e As EventArgs) Handles SearchLookUpEdit1.EditValueChanged
            ...
            Dim edit As SearchLookUpEdit = TryCast(sender, SearchLookUpEdit)

            Dim _Parte As Parte = oDTC.Parte.Where(Function(F) F.ID_Parte = CInt(edit.EditValue)).FirstOrDefault

            'MsgBox(CInt(edit.EditValue))
            'MsgBox(IsNothing(_Parte))
            'MsgBox(_Parte.ID_Parte)
            'MsgBox(Mid(_Parte.Cliente.Nombre, 1, 200))
            'MsgBox(_Parte.TrabajoARealizar)
            'MsgBox(_Parte.ID_Parte & " - [" & Mid(_Parte.Cliente.Nombre, 1, 94) & "] - " & _Parte.TrabajoARealizar)
            If _Parte Is Nothing = False Then
                Me.tbSubject.Text = _Parte.ID_Parte & " - [" & Mid(_Parte.Cliente.Nombre, 1, 80) & "] - " & Mid(_Parte.TrabajoARealizar, 1, 100)
                Me.tbLocation.Text = _Parte.Poblacion
                Me.SearchLookUpEdit1.Tag = _Parte.ID_Parte
            ...
            'Call CargarDades(rowView("xAgenda_ID"))
            ' SchedulerControl1.DayView.TimeSlots.Add(New TimeSlot(TimeSpan.FromMinutes(20), "20 minutes"))
            'Me.SchedulerControl1.DayView.TimeScale = TimeSpan.FromMinutes(15)
            'Me.SchedulerControl1.GoToDate(Now)
            'Me.SchedulerControl1.GoToToday()
        End Sub
    End Class

	Public NotInheritable Class AppointmentFormOutlook2007StyleImagesName
