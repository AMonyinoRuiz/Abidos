...
#Region "Metodes"
    ...
    Public Sub Entrada(Optional ByVal pIdParte As Integer = 0)
        ...
        Try
            ...
            If Me.C_Estado.Items.Count = 0 Then 'Si no s'han carregat els estats.. els carregarem (obertura a través de la pila)
                ...
                'Me.ToolForm.M.clsToolBar.Afegir_Boto("Numeracion", "Numeracion")
    ...
    Private Function Guardar(Optional ByVal pMostrarMissatge As Boolean = True) As Boolean
        ...
        Try
            ...
            Else
                ...
                clsParte.GuardarParte(oLinqParte)
                'oDTC.SubmitChanges()
        ...
        Try
            With oLinqParte
                ...
                If .ID_Personal.HasValue = True Then
                    Me.C_ResponsableInforme.Text = .ID_Personal
                End If
                ...
                Me.R_ObservacionesReparacion.pText = .ObservacionesTecnico
                ...
                Me.R_TrabajosRealizados.pText = .TrabajosRealizados
                Me.R_ExplicacionHorasTecnico.pText = .ExplicacionHorasTecnico
    ...
    Private Sub GetFromForm(ByRef pParte As Parte)
        Try
            With pParte
                ...
                .ObservacionesTecnico = Me.R_ObservacionesReparacion.pText
                ...
                .TrabajosRealizados = Me.R_TrabajosRealizados.pText
                .ExplicacionHorasTecnico = Me.R_ExplicacionHorasTecnico.pText
    ...
    Private Sub Cargar_Form(ByVal pID As Integer)
        Try
            ...
            oLinqParte = clsParte.CargarDadesParte(pID) ' (From taula In oDTC.Parte Where taula.ID_Parte = pID Select taula).First
            Call SetToForm()
    ...
    Private Sub CalcularHoresRealitzades()
        Try
            ...
            Me.T_HorasRealizadas.Value = clsParte.RetornaHoresRealitzades(oLinqParte.ID_Parte) 'oLinqParte.RetornaHoresRealizadas
    ...
    Private Sub CalcularEstadoParte()
        Try
            If oLinqParte.ID_Parte_Estado = EnumParteEstado.Finalizado Then
                Util.DesActivar(Me, M_Util.Enum_Controles_Activacion.TODOS_MENOS_ALGUNOS, True, Me.T_Factura, Me.DT_Fin, Me.T_PersonaQueLoFirmo, Me.GRD_Ficheros, Me.GRD_Horas)
            ...
            If oLinqParte.ID_Parte <> 0 Then
                If oLinqParte.Parte_Horas.Count > 0 Or oLinqParte.Parte_Gastos.Count > 0 Or oLinqParte.Parte_Incidencia.Count > 0 Then
                    ...
                    clsParte.CambiarEstadoParte(oLinqParte.ID_Parte, EnumParteEstado.EnCurso)
                Else
                    Me.C_Estado.Value = CInt(EnumParteEstado.Pendiente)
                    clsParte.CambiarEstadoParte(oLinqParte.ID_Parte, EnumParteEstado.Pendiente)
    ...
    Private Sub CallActualitzarEnLaBDTotsElsTotals()
        ...
        '    oDTC.SubmitChanges()
        'End If
        Call clsParte.GuardarTotals(oLinqParte.ID_Parte)
    End Sub
...
#Region "Grid Horas"
    ...
    Private Sub GRD_Generic_M_ToolGrid_ToolClickBotonsExtras2(ByRef sender As UltraWinToolbars.UltraToolbarsManager, ByRef e As UltraWinToolbars.ToolClickEventArgs, ByRef pGrid As M_UltraGrid.m_UltraGrid) Handles GRD_Horas.M_ToolGrid_ToolClickBotonsExtras2, GRD_Gastos.M_ToolGrid_ToolClickBotonsExtras2, GRD_Material.M_ToolGrid_ToolClickBotonsExtras2
        ...
        If pGrid.GRID.Selected.Rows(0).Cells("ID_Entrada_Linea").Value.ToString.Length = 0 Then
            Exit Sub
        End If

        Dim _IDEntradaLinea As Integer = pGrid.GRID.Selected.Rows(0).Cells("ID_Entrada_linea").Value
...
#Region "Revisiones"
    ...
    Private Sub GRD_Revision_M_GRID_BeforeRowUpdate(ByRef sender As Object, ByRef e As Infragistics.Win.UltraWinGrid.CancelableRowEventArgs) Handles GRD_Revision.M_GRID_BeforeRowUpdate
        Try
            ...
            Dim IDParteRevision As Integer = CInt(e.Row.Cells("ID_Parte").Value)
            Dim _ParteRevision As Parte_Revision = oDTC.Parte_Revision.Where(Function(F) F.ID_Parte = IDParteRevision).FirstOrDefault
...
#Region "Events Varis"
    ...
    Private Sub B_CambiarEstadoAEnCurso_Click(sender As Object, e As EventArgs) Handles B_CambiarEstadoAEnCurso.Click
        ...
        oLinqParte.ID_Parte_Estado = EnumParteEstado.EnCurso
        Me.C_Estado.Value = CInt(EnumParteEstado.EnCurso)
        Me.DT_Fin.Value = Now.Date
...
#Region "Grid ToDo"
    ...
    Private Sub CargarCombo_Usuario(ByVal pGrid As Infragistics.Win.UltraWinGrid.UltraGrid)
        Try

            Dim Valors As New Infragistics.Win.ValueList
            Dim oTaula As IQueryable(Of Usuario) = (From Taula In oDTC.Usuario Where Taula.Activo = True Order By Taula.NombreCompleto Select Taula)

            Dim Var As Usuario
            For Each Var In oTaula
                Valors.ValueListItems.Add(Var, Var.NombreCompleto)
            Next

            pGrid.DisplayLayout.Bands(0).Columns("Usuario").Style = ColumnStyle.DropDownList
            pGrid.DisplayLayout.Bands(0).Columns("Usuario").ValueList = Valors.Clone
